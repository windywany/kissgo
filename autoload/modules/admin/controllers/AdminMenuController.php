...
class AdminMenuController extends Controller {
    ...
    public function edit($id) {
        $menu = dbselect ( '*' )->from ( '{menu}' )->where ( array ('id' => intval ( $id ) ) );
        if (count ( $menu )) {
            $menu = $menu [0];
        } else {
            $menu = array ();
        ...
        $data ['validateRule'] = $form->rules ();
        $data ['items'] = dbselect ( '*' )->from ( '{menu_items}' )->where ( array ('parent' => 0, 'menu' => $menu ['alias'] ) )->asc ( 'sort' );
    ...
    public function setdefault($id) {
        count ( dbupdate ( '{menu}' )->set ( array ('is_default' => 0 ) )->where ( array ('id <>' => $id ) ) );
        count ( dbupdate ( '{menu}' )->set ( array ('is_default' => 1 ) )->where ( array ('id' => $id ) ) );
    ...
    public function delitem($id, $mid) {
        count ( dbdelete ()->from ( 'menu_items' )->where ( array ('id' => $id ) ) );
    ...
    function hook_for_output_menu_items($html, $items) {
        ...
        foreach ( $items as $item ) {
            ...
            $subitems = dbselect ( '*' )->from ( '{menu_items}' )->where ( array ('parent' => $item ['id'], 'menu' => $item ['menu'] ) )->asc ( 'sort' );
            if (count ( $subitems ) > 0) {
                $html .= '<ol>';
    ...
    private function save_items($items) {
        ...
        foreach ( $items as $item_id => $item ) {
            $rst = dbupdate ( '{menu_items}' )->set ( $item )->where ( array ('id' => $item_id ) );
            $rst = count ( $rst );
            if ($rst === false) {
                return false;
            }
    ...
    private function add_url_item($menu, $target) {
        ...
        $item ['sort'] = 9999;
        $item ['target'] = $target;
        $rst = dbinsert ( $item )->inito ( '{menu_items}' );
        if (count ( $rst )) {
            $item ['id'] = $rst [0];
            $items [] = $item;
        }
    ...
    private function add_node_item($menu, $target) {
        ...
        foreach ( $ids as $pid ) {
            ...
            if (dbselect ( 'id' )->from ( 'menu_items' )->where ( array ('menu' => $menu, 'type' => 'node', 'nid' => $pid ) )->count ( 'id' ) > 0) {
        ...
        if ($items) {
            $rst = dbinsert ( $items, true )->inito ( '{menu_items}' );
            if (count ( $rst )) {
                $rtn = array ();
