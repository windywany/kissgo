define('media/js/media', function(require, exports) {
    ...
    var initUploader =
            function() {
                ...
                uploader
                        .bind('UploadComplete', function(uper, fs) {
                            if (fs.length == 0)
                                return;
                            $('#attach-info').show();
                            ...
                            $
                                    .each(fs, function(i, f) {
                                        if (f.status == plupload.DONE) {
                                            ...
                                            var editor =
                                                    '<div class="plupload_filt_name"><input placeholder="描述" type="text" name="uploader_' + i + '_title" value="' + nm + '"/></div>';
                                            editor +=
                                                    '<div class="plupload_filt_alt"><input type="hidden" name="uploader_' + i + '_size" value="' + f.size + '"/>';
                ...
                var preProcessData =
                        function(data) {
                            for ( var i = 0; i < data.rows.length; i++) {
                                ...
                                data.rows[i].cell[2] =
                                        '<a href="' + KsgApp.base + 'files/' + data.rows[i].cell[7] + '/' + data.rows[i].id + '" target="_blank">' + data.rows[i].cell[2] + '</a>';
