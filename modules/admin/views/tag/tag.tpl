...
<ul class="nav nav-tabs">
    <li>&nbsp;&nbsp;</li>
    <li{if $isTag} class="active"{/if}><a href="{$_CUR_URL}"><i class="icon-tags"></i> Tags</a></li>
    <li{if !$isTag} class="active"{/if}><a href="{$_CUR_URL}?flag"><i class="icon-bookmark"></i> Flags</a></li>
</ul>
<div id="tabs">
    <div class="tab-pane active row-fluid" id="tab-tag">
        <div>
            <form class="form-inline" id="tag-form" method="get" action="{$_CUR_URL}">
                {if !$isTag}
                <input type="hidden" name="flag" id="flag-id" value=""/>
                {/if}
				<input type="text" class="span2"
					name="tag" value="{$tag}" placeholder="关键字" />
            ...
            <div class="controls pull-left">
                <div class="input-prepend input-append">
                    <span class="add-on">{if $isTag}Tag{else}Flag{/if}</span><input type="text" size="16" id="new-tag" class="input-medium"><button id="btn-add-tag" type="button" class="btn btn-add"><i class="icon-plus"></i>添加</button>
                </div>
            </div>
