$(function() {
	...
	$('.menu-del-media').click(function() {
		var me = $(this), url = me.attr('href'), ids = $('#media-list').uiTable('selected');
		if (ids.length == 0) {
			alert('请选择要删除的文件');
			return false;
		}
	...
	$('.edit-media').click(function() {
		var me = $(this), pe = me.parents('.media-info');
	...
	$('.btn-edit-att').click(function() {
		var me = $(this), pe = me.parents('.media-info'), bca = pe.find('.btn-ca-att'), icon = me.find('i');
		var name = pe.find('.a_name').val(), alt = pe.find('.a_alt').val(), url = pe.find('.edit-media').attr('href');
