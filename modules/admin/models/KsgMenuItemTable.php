...
class KsgMenuItemTable extends DbTable {
    ...
    public function schema() {
        ...
        $schema ['is_navi'] = array ('type' => 'bool', 'extra' => 'normal', Idao::NN, Idao::DEFT => 1, Idao::CMMT => '是不是导航' );
    ...
    public function getMenuItems($menu_name) {
        $nodes = $this->query ()->where ( array ('menu_name' => $menu_name, 'is_navi' => 1 ) )->sort ( 'sort', 'a' );
        return $nodes;
    }
    public function getSubItems($menu_name, $up_id) {
        $nodes = $this->query ()->where ( array ('menu_name' => $menu_name, 'is_navi' => 1, 'up_id' => $up_id ) )->sort ( 'sort', 'a' );
        return $nodes;
    }
