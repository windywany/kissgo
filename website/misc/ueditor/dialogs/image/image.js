...
(function () {
    ...
    var flagImg = null, flashContainer,args = {pid:1,alt:'',year:'',month:'',type:''},totalPageCount = 1;
            ...
            if (id != tabSrc) {
                ...
                if (id == "imgManager") {
                    ...
                    if (!list.children.length) {
                    	...
                    	getImageList(editor.options.imageManagerUrl);
                    }
                }
                if (id == "imgSearch") {
    ...
    function getImageList(url){
    	g( "imageList" ).innerHTML = "<p id='imgTip'>正在加载，请稍等....</p>";
    	ajax.request(url, {
            timeout:100000,
            action:"get",
            onsuccess:function ( xhr ) {
                var tmp = xhr.responseText;
                var imageUrls = [],length=0;
                if(tmp){
                	var pos = tmp.indexOf('ue_page_ue');
                	if(pos > 0){
                		totalPageCount = tmp.substring(0,pos);
                		tmp = tmp.substring(pos+10);
                		g('pageInfo').innerHTML = '当前在第<strong>'+args.pid+'</strong>页,共<strong>'+totalPageCount+'</strong>页,每页20张图片';
                	}
                	if(tmp){
	                	imageUrls = tmp.split( "ue_separate_ue" ),
	                	length = imageUrls.length;
                	}
                }       
                g( "imageList" ).innerHTML = !length ? "<p id='imgTip'>未找到图片,请先上传！</p>" : "";
                ...
                for ( var i = 0, ci; ci = imageUrls[i++]; ) {
                    ...
                    img.setAttribute( i < 35 ? "src" : "lazy_src", editor.options.imagePath + ci );
                    img.setAttribute( "data_ue_src", editor.options.imagePath + ci );

                }
            },
            onerror:function () {
                g( "imageList" ).innerHTML = "<p id='imgTip'>糟糕，图片读取失败了！<p>";
            }
        } );    	
    }
    function getImageListUrl(){
    	var url = editor.options.imageManagerUrl,sep='?';
    	if(url.indexOf('?')>=0){
    		sep = '&';
    	}
    	url += sep+'type='+args.type+'&title='+args.alt+'&year='+args.year+'&month='+args.month+'&pid='+(args.pid?args.pid:1);
    	return url;
    }
    window.doImageSearch = function(){
    	args.year = g('sYear').value;
    	args.month=g('sMonth').value;
    	args.alt=g('img_alt').value;
    	args.type = g('sType').value;
    	args.pid = 1;
    	if(args.alt){
    		args.alt = encodeURIComponent(args.alt);
    	} 
    	getImageList(getImageListUrl());
    };
    window.gotePage = function(p){    	
    	var pid = args.pid + p;
    	if(pid >= 1 && pid <= totalPageCount){
    		args.pid = pid;    		
        	getImageList(getImageListUrl());
    	}    	
    };
